...
function GraphCandles({
  ...
  //highlight open close diffrence
  ...
  useEffect(() => {
    if (chartContainerRef.current && selectedData.length) {
      ...
      function handleMouseMove(param: MouseEventParams): void {
        if (param) {
          ...
          if (param.time) {
            let hoveredOverData =
              selectedData[
                selectedData.findIndex((el) => el.time == param.time)
              ];
      ...
      if (keys.includes("time") && keys.length > 4) {
        ...
        let markDataN1 = selectedData.filter((d) => d["ml_signal"] == -1);
        ...
        if (highlightOpenClose > 0) {
          console.log(highlightOpenClose);
          let selectedDataCopy = selectedData.map((el) => {
            return {
              ...el,
              color:
                Math.abs(el.open - el.close) > highlightOpenClose
                  ? "orange"
                  : null,
              wickColor:
                Math.abs(el.open - el.close) > highlightOpenClose
        ...
        <div className="div--graph-historical-hover-over-data-wrapper">
          ...
          <ul className="ul--graph-historical-hover-over-data">
            <li>
              ...
              {
                //@ts-ignore
                hoverOverValue.open ? hoverOverValue.open.toFixed(2) : null
              }
            </li>
            <li>
              High:
              {
                //@ts-ignore
                hoverOverValue.high ? hoverOverValue.high.toFixed(2) : null
              }
            </li>
            <li>
              Low:
              {
                //@ts-ignore
                hoverOverValue.low ? hoverOverValue.low.toFixed(2) : null
              }
            </li>
            <li>
              Close:
              {
                //@ts-ignore
                hoverOverValue.close ? hoverOverValue.close.toFixed(2) : null
              }
            </li>
            <li>
              Volume:
              {
                //@ts-ignore
                hoverOverValue.volume ? hoverOverValue.volume.toFixed(2) : null
              }
            </li>
            <li>
              Signal:
              {
                //@ts-ignore
                hoverOverValue.ml_signal ? hoverOverValue.ml_signal : null
              }
            </li>
          </ul>
        </div>
